<template>
  <div class="home">
      ...
      <div :class="['header-box', isStandardType ? 'standard-header-box' : 'demo-header-box']">
        <div class="apply-status">
          <div v-if="entryInfo.state === 0">
            <div v-if="entryInfo.isOwn">我已报名</div>
            <div v-else>
              <span class="user-name">{{entryInfo.nickName}}</span>
              <span>已报名</span>
            </div>
          </div>
          ...
          <div class="content-title">
            <div v-if="entryInfo.state === 0">
              ...
              <span v-else>
                ...
                <div class="time_tip">:</div>
                <div class="time_box">{{dateObj.seconds}}</div>
                <div class="time_tip">后好友砍价活动结束</div>
              </span>
            </div>
            <div v-else>好友{{entryInfo.nickName}}砍价活动已结束</div>
            <div v-if="entryInfo.state === 0" class="share" @click="sharePoster"></div>
          </div>
          <div class="small-content-box">
            <div :class="isStandardType ? 'standard-apply-link' : 'demo-apply-link' ">
              <div v-if="isStandardType" class="apply-btn" @click="getApplyLink">立即0元领</div>
              <template v-else>
                <div
                  class="apply-btn"
                  @click="getApplyLink"
                >{{this.applyInfo.applyMoney |filterMoney }}元立即领取</div>
                <div class="apply-tip"></div>
              </template>
            </div>
          </div>
        </div>
    ...
    <bcustomer-dialog
      ref="BcustomerDialog"
      @updateHome="getBargainInfo"
      @gotoCourse="getApplyLink"
    />
    <!-- B---退费或超时 -->
    <berror-dialog ref="BerrorDialog" @gotoCourse="getApplyLink" />
    <!-- B---当天已砍 -->
    <Bkan ref="Bkaned" @gotoPoster="sharePoster" />
  </div>
</template>
...
import BcustomerDialog from './dialog/Bcustomer/Bindex'
import BerrorDialog from './dialog/Bcustomer/error'
import Bkan from './dialog/Bcustomer/kaned'
...
export default {
  ...
  methods: {
    init() {
      ...
      const gotoUrl = window.location.origin + window.location.pathname + '#/friend'
      CommonLoginSDK.getWxUserInfo(ENV, gotoUrl).then(res => {
        this.wxUserInfo = res
        window.sessionStorage.setItem('wxUserInfo', JSON.stringify(res))
        this.getBargainInfo()
      })
    },
    getBargainInfo() {
    ...
    openDialog() {
      ...
      if (this.entryInfo.isOwn) {
        // 你不能给自己砍刀
        this.$refs.Bkaned.show(this.entryInfo, '你不能给自己砍刀！')
      } else {
        // 正常
        if (this.entryInfo.state === 0) {
          // B--符合条件-首次进入（今天未砍刀）
          if (!this.entryInfo.isBargain) {
            this.$refs.BcustomerDialog.show(this.entryInfo)
          } else {
            this.$refs.Bkaned.show(this.entryInfo, '哎呀，您今日已经砍过啦！')
          }
        } else if (this.entryInfo.state === 1) {
          this.$refs.BerrorDialog.show('timeout', this.entryInfo)
        } else {
          // 异常
          // B--A已申请退费或退费中
          this.$refs.BerrorDialog.show('error', this.entryInfo)
        }
      }
    },
    getApplyInfo() {
